...
public class HotUpdateModuleManager : SingletonTemplate<HotUpdateModuleManager>
{
    ...
    public void doObtainServerVersionConfig(Action<bool> completecallback)
    {
        ...
        //URL = 热更新URL + 服务器版本信息文件名(ServerVersionConfig.json) //还需要知道是什么平台下的
        var url = mHotUpdateURL +URL.GetPlatformStr()+"/"+ ServerVersionConfigFileName; ;
...
#region 资源热更新部分
    ...
    public void doResourceHotUpdate(int newresourcehotupdate, Action<bool> completecallback)
    {
        ...
        //拉取服务器热更资源信息与本地资源热更信息进行比较
        TWebRequest twr = new TWebRequest();
        //URL = 基础URL + 当前版本号 + "/" + 热更资源MD5信息文件名(AssetBundleMD5.txt) //还需要和平台相关
        var url = $"{mHotUpdateURL}{URL.GetPlatformStr()}/{ServerVersionConfig.VersionCode}/{ServerVersionConfig.ResourceVersionCode}/{AssetBundleMD5FileName}";
            ...
            try
            {
                ...
                if(mHotUpdateResourceTotalNumber > 0)
                {
                    ...
                    foreach (var resinfo in mNeedHotUpdateAssetBundleInfoMap)
                    {
                        ...
                        var finalurl = $"{mHotUpdateURL}{URL.GetPlatformStr()}/{ServerVersionConfig.VersionCode}/{ServerVersionConfig.ResourceVersionCode}/{resinfo.Value.AssetBundlePath}";
    ...
    private void singleResourceHotUpdateCompleteCB(string url, string fileMd5, DownloadHandler downloadhandler, TWebRequest.WebRequestTaskInfo.WebTaskRequestStatus requeststatus)
    {
        ...
        var hotUpdatePreURL = $"{mHotUpdateURL}{URL.GetPlatformStr()}/{hotUpdateVersionCode}/{hotUpdateResourceVersionCode}/";
