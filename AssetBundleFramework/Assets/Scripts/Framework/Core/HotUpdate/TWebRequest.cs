...
public class TWebRequest {
    ...
    /// <summary>
    /// 任务请求携程,可以顺序批量请求，这里需要增加一个断点续传的逻辑，不同情况下的调用也要做些区别，超时时间需要做调整
    /// </summary>
    /// <returns></returns>
    private IEnumerator requestCoroutine()
    {
        ...
        while (mWebRequestTaskQueue.Count > 0 && TWRequestStatus == TWebRequestStatus.TW_In_Progress)
        {
            ...
            if (mCurrentInProgressWebRequest.result== UnityWebRequest.Result.ConnectionError || mCurrentInProgressWebRequest.result == UnityWebRequest.Result.ProtocolError)
            {
                Debug.LogError($"{task.URL}资源下载出错!");
                Debug.LogError(mCurrentInProgressWebRequest.error);
                if (mCurrentInProgressWebRequest.result == UnityWebRequest.Result.ProtocolError)
                {
