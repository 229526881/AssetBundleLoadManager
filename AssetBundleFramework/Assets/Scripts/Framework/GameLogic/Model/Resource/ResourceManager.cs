...
public class ResourceManager : SingletonTemplate<ResourceManager>
{
    ...
    {
        TResource.AssetLoader assetLoader;
        int id= TResource.ResourceModuleManager.Singleton.requstAssetSync<GameObject>(
            respath,
            out assetLoader,
            (loader, requestUid) =>
            {
    ...
    public GameObject GetPrefabInstance(string respath,  TResource.ResourceLoadType loadtype = TResource.ResourceLoadType.NormalLoad)
    {
        TResource.AssetLoader assetLoader;
        int id = TResource.ResourceModuleManager.Singleton.requstAssetSync<GameObject>(
            respath,
        ...
        var prefab = assetLoader.obtainAsset<GameObject>();
        var prefabinstance = UnityEngine.Object.Instantiate<GameObject>(prefab);
        assetLoader.bindAsset<GameObject>(prefabinstance);
#if UNITY_EDITOR
        ResourceUtility.FindMeshRenderShaderBack(prefabinstance);
#endif
    ...
    {
        return TResource.ResourceModuleManager.Singleton.requstAssetAsync<AudioClip>(
            respath,
            out assetLoader,
            (loader, requestUid) =>
            {
    ...
    /// <param name="loadtype"></param>
    /// <returns></returns>
    public T LoadAssetSync<T>(UnityEngine.Object owner,string path, TResource.ResourceLoadType loadtype = TResource.ResourceLoadType.NormalLoad)where T: UnityEngine.Object
    {
        TResource.AssetLoader assetLoader;
        TResource.ResourceModuleManager.Singleton.requstAssetSync<T>(
            path,
            out assetLoader,
            null,
            loadtype
        );
        T obj = assetLoader.bindAsset<T>(owner);
        return obj;
    }
